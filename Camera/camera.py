...
The folder has already been created")
    ...
    def stream(self, 
        ...
        writer = None
        if self.__mode == "video":
            writer = self._video_writer(path, fps, frame)
...
if __name__ == "__main__":
    cam = Camera(0, "frame")
    rtsp = Camera("rtsp://grant:QWERTYUIOP{}@192.168.18.25:554/cam/realmonitor?channel=1&subtype=1", "frame")
    # cam.stream(fps=9, img_count=3)
    # rtsp.stream(fps=30)
    ...
    # path: str = "./",
    #fps: int = 15
    # barier: multiprocessing.Barrier = None
    
    # Парлельный запуск камер
    barier = multiprocessing.Barrier(2)
    ...
    cam_proc = multiprocessing.Process(target=cam.stream, kwargs=kwargs_)
